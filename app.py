    ...
    for t in soup.find_all('table'):
        ...
        #                        bach = td.string
                            ...
                            if bach in teamDict.keys():
                                ...
                                if week <= 1:
                                    points = 0
                                results.append([team,bach,pick,'Week ' + str(week),week,pointsForRose,pointsForOrder,pointsForBonus])
                        week += 1
        tableCount += 1 
    
    resultsDf = pd.DataFrame(results,columns=['Team','Bachelor','Pick','Week','Week Number','Rose','Order','Bonus']).sort_values(by=['Week Number','Team','Pick'])
    
    #resultsDf[resultsDf['Bachelor'].str.contains('Christen')]
    #resultsDf.to_csv('/Users/JM_JM/Desktop/auto_bachelor_data.csv')
    
    detailDf = resultsDf[(resultsDf['Week']!='Week 0') & (resultsDf['Week']!='Week 1')].groupby(by=['Team','Week Number','Bachelor']).sum()
    del detailDf['Pick']
    detailDf.reset_index(level=[0,1,2],inplace=True)
    detailDf = detailDf.pivot(index='Team', columns='Week Number', values='Rose').fillna(0)
